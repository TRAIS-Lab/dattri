...
class TestRetrainLDS(unittest.TestCase):
    ...
    def test_retrain_lds_saves_metadata_correctly(self):
        ...
        with patch("yaml.dump") as mock_yaml_dump:
            ...
            assert saved_metadata["subset_number"] == 2, "Incorrect subset number in metadata"
            assert len(saved_metadata["map_subset_dir"]) == 2, "Incorrect map subset directory count"
...
class TestCalculateLDSTest(unittest.TestCase):
    ...
    def test_calculate_lds_groundtruth(self, mock_join, mock_listdir):
        ...
        assert self.target_func.call_count == 3, "Target function was not called the expected number of times"
...
class TestLDS(unittest.TestCase):
    ...
    def test_lds_computation(self, mock_spearmanr):
        ...
        assert torch.allclose(lds_values, expected_lds_values, atol=1e-6), "Incorrect LDS values computed"
        assert mock_spearmanr.call_count == 3, "Spearmanr function call count is incorrect"
    ...
    def test_lds_identical_scores(self, mock_spearmanr):
        ...
        assert torch.isnan(lds_values).all(), "LDS values should be NaN for identical scores"
